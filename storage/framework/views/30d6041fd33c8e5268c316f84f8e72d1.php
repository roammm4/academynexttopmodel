...
<body>
  ...
  <?php if($selected): ?>
    ...
    <?php echo e($selected->experience_value); ?> <?php echo e(__('messages.portofolio_' . ($selected->experience_unit ?? 'years'))); ?> <?php echo e(__('messages.portofolio_exp')); ?>
    ...
    <?php echo e($selected->status == 'available' ? __('messages.portofolio_available') : __('messages.portofolio_not_available')); ?>
          ...
          <div class="flex items-center gap-3 mt-2">
            <a href="<?php echo e(route('models.edit', $selected->id_model)); ?>" class="inline-block bg-white text-black text-[12px] font-semibold px-[12px] py-[6px] rounded-full hover:bg-gray-200"><?php echo e(__('messages.portofolio_edit_model')); ?></a>
            <button type="button" onclick="confirmDelete(<?php echo e($selected->id_model); ?>)" class="inline-block bg-red-600 text-white text-[12px] font-semibold px-[12px] py-[6px] rounded-full hover:bg-red-800 ml-2">
                <?php echo e(__('messages.portofolio_delete_model')); ?>
    ...
    <div class="aspect-square flex-1 bg-gray-700 flex items-center justify-center relative overflow-hidden">
      <?php if(isset($portfolioSlots[1]) && $portfolioSlots[1]): ?>
        ...
        <div class="absolute top-2 right-2 flex gap-2 z-10">
          <!-- Edit Button -->
          <form action="<?php echo e(route('portofolio.update', [$selected->id_model ?? $model->id_model, 1])); ?>" method="POST" enctype="multipart/form-data" id="edit-form-1">
            <?php echo csrf_field(); ?>
            <input type="file" name="photo" accept="image/*" class="hidden" onchange="this.form.submit()" id="edit-upload-1">
            <label for="edit-upload-1" class="cursor-pointer bg-white text-black px-2 py-1 rounded text-xs font-bold"><?php echo e(__('messages.portofolio_edit')); ?></label>
          </form>
          <!-- Delete Button -->
          <form action="<?php echo e(route('portofolio.delete', [$selected->id_model ?? $model->id_model, 1])); ?>" method="POST" id="delete-portfolio-form-1">
            <?php echo csrf_field(); ?>
            <?php echo method_field('DELETE'); ?>
            <button type="button" onclick="confirmDeletePortfolio(1)" class="bg-red-600 text-white px-2 py-1 rounded text-xs font-bold"><?php echo e(__('messages.portofolio_delete')); ?></button>
          </form>
        </div>
      <?php endif; ?>
      <form action="<?php echo e(route('portofolio.upload', [$selected->id_model ?? $model->id_model, 1])); ?>" method="POST" enctype="multipart/form-data" class="absolute inset-0 flex items-center justify-center" style="<?php echo e(isset($portfolioSlots[1]) && $portfolioSlots[1] ? 'display:none;' : ''); ?>">
        <?php echo csrf_field(); ?>
    ...
    <div class="grid grid-cols-2 grid-rows-2 gap-4 flex-1">
      ...
      <div class="aspect-square bg-gray-700 flex items-center justify-center relative overflow-hidden">
        <?php if(isset($portfolioSlots[$i]) && $portfolioSlots[$i]): ?>
          ...
          <div class="absolute top-2 right-2 flex gap-2 z-10">
            <form action="<?php echo e(route('portofolio.update', [$selected->id_model ?? $model->id_model, $i])); ?>" method="POST" enctype="multipart/form-data" id="edit-form-<?php echo e($i); ?>">
              <?php echo csrf_field(); ?>
              <input type="file" name="photo" accept="image/*" class="hidden" onchange="this.form.submit()" id="edit-upload-<?php echo e($i); ?>">
              <label for="edit-upload-<?php echo e($i); ?>" class="cursor-pointer bg-white text-black px-2 py-1 rounded text-xs font-bold"><?php echo e(__('messages.portofolio_edit')); ?></label>
            </form>
            <form action="<?php echo e(route('portofolio.delete', [$selected->id_model ?? $model->id_model, $i])); ?>" method="POST" id="delete-portfolio-form-<?php echo e($i); ?>">
              <?php echo csrf_field(); ?>
              <?php echo method_field('DELETE'); ?>
              <button type="button" onclick="confirmDeletePortfolio(<?php echo e($i); ?>)" class="bg-red-600 text-white px-2 py-1 rounded text-xs font-bold"><?php echo e(__('messages.portofolio_delete')); ?></button>
            </form>
          </div>
        <?php endif; ?>
        <form action="<?php echo e(route('portofolio.upload', [$selected->id_model ?? $model->id_model, $i])); ?>" method="POST" enctype="multipart/form-data" class="absolute inset-0 flex items-center justify-center" style="<?php echo e(isset($portfolioSlots[$i]) && $portfolioSlots[$i] ? 'display:none;' : ''); ?>">
          <?php echo csrf_field(); ?>
          <input type="file" name="photo" accept="image/*" class="hidden" onchange="this.form.submit()" id="upload-<?php echo e($i); ?>">
          <label for="upload-<?php echo e($i); ?>" class="cursor-pointer text-4xl text-white bg-gray-800 bg-opacity-70 rounded-full w-14 h-14 flex items-center justify-center hover:bg-gray-600">+</label>
        </form>
      </div>
      <?php endfor; ?>
    </div>
  ...
  <div class="flex gap-4">
    ...
    <div class="aspect-square flex-1 bg-gray-700 flex items-center justify-center relative overflow-hidden">
      <?php if(isset($portfolioSlots[10]) && $portfolioSlots[10]): ?>
        ...
        <div class="absolute top-2 right-2 flex gap-2 z-10">
          <form action="<?php echo e(route('portofolio.update', [$selected->id_model ?? $model->id_model, 10])); ?>" method="POST" enctype="multipart/form-data" id="edit-form-10">
            <?php echo csrf_field(); ?>
            <input type="file" name="photo" accept="image/*" class="hidden" onchange="this.form.submit()" id="edit-upload-10">
            <label for="edit-upload-10" class="cursor-pointer bg-white text-black px-2 py-1 rounded text-xs font-bold"><?php echo e(__('messages.portofolio_edit')); ?></label>
          </form>
          <form action="<?php echo e(route('portofolio.delete', [$selected->id_model ?? $model->id_model, 10])); ?>" method="POST" id="delete-portfolio-form-10">
            <?php echo csrf_field(); ?>
            <?php echo method_field('DELETE'); ?>
            <button type="button" onclick="confirmDeletePortfolio(10)" class="bg-red-600 text-white px-2 py-1 rounded text-xs font-bold"><?php echo e(__('messages.portofolio_delete')); ?></button>
          </form>
        </div>
      <?php endif; ?>
      <form action="<?php echo e(route('portofolio.upload', [$selected->id_model ?? $model->id_model, 10])); ?>" method="POST" enctype="multipart/form-data" class="absolute inset-0 flex items-center justify-center" style="<?php echo e(isset($portfolioSlots[10]) && $portfolioSlots[10] ? 'display:none;' : ''); ?>">
        <?php echo csrf_field(); ?>
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
            ...
            <div class="w-full h-[450px] bg-gray-700 flex items-center justify-center relative overflow-hidden">
              <?php if(isset($careerSlots[$i]) && $careerSlots[$i]): ?>
                ...
                <div class="absolute top-2 right-2 flex gap-2 z-10">
                  ...
                  <button type="button" class="bg-white text-black px-2 py-1 rounded text-xs font-bold" onclick="openCareerEditModal(<?php echo e($i); ?>, '<?php echo e($careerSlots[$i]->event_name); ?>', '<?php echo e($careerSlots[$i]->year); ?>')"><?php echo e(__('messages.portofolio_edit')); ?></button>
                  <form action="<?php echo e(route('career.delete', [$selected->id_model ?? $model->id_model, $i])); ?>" method="POST" id="delete-career-form-<?php echo e($i); ?>">
                    <?php echo csrf_field(); ?>
                    <?php echo method_field('DELETE'); ?>
                    <button type="button" onclick="confirmDeleteCareer(<?php echo e($i); ?>)" class="bg-red-600 text-white px-2 py-1 rounded text-xs font-bold"><?php echo e(__('messages.portofolio_delete')); ?></button>
                  </form>
                </div>
              ...
              <?php endif; ?>
              <form action="<?php echo e(route('career.upload', [$selected->id_model ?? $model->id_model, $i])); ?>" method="POST" enctype="multipart/form-data" class="absolute inset-0 flex items-center justify-center" style="<?php echo e(isset($careerSlots[$i]) && $careerSlots[$i] ? 'display:none;' : ''); ?>" id="career-upload-form-<?php echo e($i); ?>">
                <?php echo csrf_field(); ?>
                <input type="file" name="photo" accept="image/*" class="hidden" onchange="handleCareerFileChange(event, <?php echo e($i); ?>)" id="career-upload-<?php echo e($i); ?>">
                <input type="hidden" name="event_name">
                <input type="hidden" name="year">
                <label for="career-upload-<?php echo e($i); ?>" class="cursor-pointer text-4xl text-white bg-gray-800 bg-opacity-70 rounded-full w-14 h-14 flex items-center justify-center hover:bg-gray-600">+</label>
              </form>
            </div>
            <?php endfor; ?>
          </div>
              ...
              <?php if(isset($awardSlots[$i]) && $awardSlots[$i]): ?>
                ...
                <div class="absolute top-2 right-2 flex gap-2 z-10">
                  <form action="<?php echo e(route('award.update', [$selected->id_model ?? $model->id_model, $i])); ?>" method="POST" enctype="multipart/form-data" id="edit-award-form-<?php echo e($i); ?>">
                    ...
                    <label for="edit-award-upload-<?php echo e($i); ?>" class="cursor-pointer bg-white text-black px-2 py-1 rounded text-xs font-bold"><?php echo e(__('messages.portofolio_edit')); ?></label>
                  </form>
                  <form action="<?php echo e(route('award.delete', [$selected->id_model ?? $model->id_model, $i])); ?>" method="POST" id="delete-award-form-<?php echo e($i); ?>">
                    <?php echo csrf_field(); ?>
                    <?php echo method_field('DELETE'); ?>
                    <button type="button" onclick="confirmDeleteAward(<?php echo e($i); ?>)" class="bg-red-600 text-white px-2 py-1 rounded text-xs font-bold"><?php echo e(__('messages.portofolio_delete')); ?></button>
                  </form>
                </div>
              <?php endif; ?>
              <form action="<?php echo e(route('award.upload', [$selected->id_model ?? $model->id_model, $i])); ?>" method="POST" enctype="multipart/form-data" class="absolute inset-0 flex items-center justify-center" style="<?php echo e(isset($awardSlots[$i]) && $awardSlots[$i] ? 'display:none;' : ''); ?>">
                <?php echo csrf_field(); ?>
                <input type="file" name="photo" accept="image/*" class="hidden" onchange="this.form.submit()" id="award-upload-<?php echo e($i); ?>">
                <label for="award-upload-<?php echo e($i); ?>" class="cursor-pointer text-4xl text-white bg-gray-800 bg-opacity-70 rounded-full w-14 h-14 flex items-center justify-center hover:bg-gray-600">+</label>
              </form>
            </div>
            <?php endfor; ?>
          </div>
      ...
      <div class="text-[14px] text-gray-300 mb-[40px]">
        <h3 class="text-[22px] text-white mb-[15px] font-fondamento"><?php echo e(__('messages.portofolio_contact_booking')); ?></h3>
        <p><?php echo e(__('messages.portofolio_office')); ?><br />
        <?php echo e(__('messages.portofolio_address')); ?></p>
        <p><?php echo e(__('messages.portofolio_wa')); ?></p>
        <p><?php echo e(__('messages.portofolio_ig')); ?></p>
        <p><?php echo e(__('messages.portofolio_email')); ?></p>
      </div>
      <!-- Footer -->
      <footer class="flex justify-between text-[12px] text-gray-500 border-t border-[#333] pt-[15px]">
        <span><?php echo e(__('messages.portofolio_footer')); ?></span>
        <span><?php echo e(__('messages.portofolio_instagram')); ?></span>
      </footer>
    </section>
  <?php endif; ?>
</body>
</html>
<?php /**PATH C:\xampp\htdocs\academyntm2-main\resources\views/portofolio.blade.php ENDPATH**/ ?>